...
import { createNode, resetMsgTop } from './utils'
...
const createMessage = (cfg: object) => {
    ...
    messageNode.style.top = messageList.length * height - height + 'px'
    ...
    const app: any = createApp(Message, {
        ...
        remove() {
            ...
            //移除
            gsap.to(messageNode, {
                top: messageNode.offsetTop - height,
                opacity: 0,
                duration: time / 1000,
                ease: 'Power1.easeOut'
            }).then(() => {
